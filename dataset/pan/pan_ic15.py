    ...
    with open(gt_path, encoding='utf-8', mode='r') as f:
        for line in f.readlines():
            ...
            distance = (math.sqrt((bbox[2] - bbox[0]) ** 2 + (bbox[3] - bbox[1]) ** 2) + math.sqrt(
                (bbox[6] - bbox[4]) ** 2 + (bbox[7] - bbox[5]) ** 2)) / (4 * max(h, w))
            # print(distance)
            bbox = np.array(bbox) / ([w * 1.0, h * 1.0] * 4)
            bboxes.append(bbox)
            max_dinstances.append(distance)
            # print('%s是' %gt_path)
            # print(np.array(bboxes))
        return np.array(bboxes), words, max_dinstances
...
class PAN_IC15(data.Dataset):
    ...
    def prepare_train_data(self, index):
        ...
        b = [0 for i in range(self.max_word_num - t + 1)]
        # b_t = torch.tensor(b, dtype=torch.float32)
        max_distances = max_distances + b
