...
lsvt_root_dir = './data/LSVT/'
lsvt_train_data_dir = lsvt_root_dir + 'train_full_images/'
lsvt_train_gt_dir = lsvt_root_dir + 'train_gt_trans/'
lsvt_test_data_dir = 'D:/wenziji/LSVT/test/'
lsvt_test_gt_dir = lsvt_root_dir
...
def get_ann(img, gt_path):
    ...
    for line in lines:
        ...
        gt = line.split(',')
        if len(gt) >= 8 and len(gt) < 16:
            bbox = [np.int(gt[i]) for i in range(8)]
        elif len(gt) >= 16 and len(gt) < 24:
            bbox = [np.int(gt[i]) for i in range(16)]
        elif len(gt) >= 24:
            bbox = [np.int(gt[i]) for i in range(24)]
        # print(bbox)
        word = gt[-1]
        if word == '#':
            word = '###'
        if len(bbox) == 8:
            distance = math.sqrt((bbox[4] - bbox[0]) ** 2 + (bbox[5] - bbox[1]) ** 2) / (2 * (max(h, w)))
        elif len(bbox) == 16:
            distance = math.sqrt((bbox[8] - bbox[0]) ** 2 + (bbox[9] - bbox[1]) ** 2) / (2 * (max(h, w)))
        elif len(bbox) == 24:
            distance = math.sqrt((bbox[12] - bbox[0]) ** 2 + (bbox[13] - bbox[1]) ** 2) / (2 * (max(h, w)))
        else:
            distance = 50 / (2 * (max(h, w)))
        bbox = np.asarray(bbox) / ([w * 1.0, h * 1.0] * (len(bbox) // 2))
        bboxes.append(bbox)
        words.append(word)
...
class PAN_LSVT(data.Dataset):
    def __init__(self,
        ...
        if split == 'train':
            data_dirs = [lsvt_train_data_dir]
            gt_dirs = [lsvt_train_gt_dir]
        elif split == 'test':
            data_dirs = [lsvt_test_data_dir]
            gt_dirs = [lsvt_test_gt_dir]
        else:
