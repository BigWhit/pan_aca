...
mlt_root_dir = './data/MLT/'
mlt_train_data_dir = mlt_root_dir + 'train/'
mlt_train_gt_dir = mlt_root_dir + 'train_gt/'
mlt_test_data_dir = mlt_root_dir + 'test'
mlt_test_gt_dir = mlt_root_dir
...
def get_ann(img, gt_path):
    ...
    with open(gt_path, encoding='utf-8', mode='r') as f:
        ...
        for line in f.readlines():
            ...
            side = max(math.sqrt((bbox[2] - bbox[0]) ** 2 + (bbox[3] - bbox[1]) ** 2),
                       math.sqrt((bbox[4] - bbox[2]) ** 2 + (bbox[5] - bbox[3]) ** 2),
                       math.sqrt((bbox[4] - bbox[6]) ** 2 + (bbox[5] - bbox[7]) ** 2),
                       math.sqrt((bbox[6] - bbox[0]) ** 2 + (bbox[7] - bbox[1]) ** 2))
            distance = side / (2 * max(h, w))
        ...
        # print(words)
        # print(distances)
        return np.array(bboxes), words, distances
...
class PAN_MLT(data.Dataset):
    def __init__(self,
        ...
        if split == 'train':
            data_dirs = [mlt_train_data_dir]
            gt_dirs = [mlt_train_gt_dir]
        elif split == 'test':
            data_dirs = [mlt_test_data_dir]
            gt_dirs = [mlt_test_gt_dir]
        else:
    ...
    def prepare_train_data(self, index):
        ...
        bboxes, words, distances = get_ann(img, gt_path)
        t = len(distances)
        if bboxes.shape[0] > self.max_word_num:
            bboxes = bboxes[:self.max_word_num]
            words = words[:self.max_word_num]
            distances = distances[:self.max_word_num]
