...
# In addition to this API, please download both
...
#  COCO_Text - COCO-Text api class that loads COCO annotations
...
# Code based on Microsoft COCO Toolbox Version 1.0 by Piotr Dollar and
# Tsung-Yi Lin extended and adapted by Andreas Veit, 2016.
# Licensed under the Simplified BSD License [see bsd.txt]


class COCO_Text:
    def __init__(self, annotation_file=None):
        """Constructor of COCO-Text helper class for reading and visualizing
        annotations.
    ...
    def filtering(self, filterDict, criteria):
        return [
            key for key in filterDict if all(
                criterion(filterDict[key]) for criterion in criteria)
        ]
    ...
    def getAnnByCat(self, properties):
        ...
        return self.filtering(
            self.anns,
            [lambda d, x=a, y=b: d[x] == y for (a, b) in properties])
    ...
    def getAnnIds(self, imgIds=[], catIds=[], areaRng=[]):
        ...
        if len(imgIds) == len(catIds) == len(areaRng) == 0:
            ...
            if not len(imgIds) == 0:
                ...
                anns = list(self.anns.keys())
            anns = anns if len(catIds) == 0 else list(
                set(anns).intersection(set(self.getAnnByCat(catIds))))
            anns = anns if len(areaRng) == 0 else [
                ann for ann in anns if self.anns[ann]['area'] > areaRng[0]
    ...
    def showAnns(self, anns, show_polygon=False):
        ...
        for ann in anns:
            ...
            if show_polygon:
                ...
                verts = [(tl_x, tl_y), (tr_x, tr_y), (br_x, br_y),
                         (bl_x, bl_y), (0, 0)]
                codes = [
    ...
    def loadRes(self, resFile):
        ...
        if set(annsImgIds) != (set(annsImgIds) & set(self.getImgIds())):
            ...
            print(
                ...
                str(
                    len(set(annsImgIds)) -
                    len(set(annsImgIds) & set(self.getImgIds()))), ' images')
