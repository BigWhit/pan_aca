...
class PA_Head(nn.Module):
    ...
    def get_results(self, out, img_meta, cfg):
        ...
        if not self.training and cfg.report_speed:
            ...
            start = time.time()
        # print(out.size())#[1,6,160,248]
        score = torch.sigmoid(out[:, 0, :, :])
        # print(score.size())#[1,160,248]
        kernels = out[:, :2, :, :] > 0
        text_mask = kernels[:, :1, :, :]
        ...
        # print('org_img_size', org_img_size[0], org_img_size[1])#332,500
        # print('img_size', img_size[0], img_size[1])#640,992
    ...
    def loss(self, out, gt_texts, gt_kernels, training_masks, gt_instances, gt_bboxes, max_distances):  # max_distances
        # output
        # print(out.size())  # [8,6,640,640]#一次处理8张图片
        texts = out[:, 0, :, :]
        # print('texts', texts.size())  # [8,640,640]
        kernels = out[:, 1:2, :, :]
        # print(kernels.size())  # [8,1,640,640]
        embs = out[:, 2:, :, :]
        # print('embs', embs.size())  # [8,4,640,640]

        # text loss
        selected_masks = ohem_batch(texts, gt_texts, training_masks)
        loss_text = self.text_loss(texts, gt_texts, selected_masks, reduce=False)  # 加入miu
        ...
        for i in range(kernels.size(1)):
            ...
            loss_kernel_i = self.kernel_loss(kernel_i, gt_kernel_i, selected_masks, reduce=False)  # 加入miu
        ...
        # embedding loss
        loss_emb = self.emb_loss(embs, gt_instances, gt_kernels[:, -1, :, :], training_masks, gt_bboxes, max_distances,
                                 reduce=False)  # max_distances,
        losses.update(dict(
            loss_emb=loss_emb
        ))
