...
#tensorboard --logdir=D:\liuqian\pan_pp.pytorch\tensorboard
...
def train(train_loader, model, optimizer, epoch, start_iter, cfg):
    ...
    start = time.time()
    # tensorboard
    writer = SummaryWriter('./tensorboard/mlt_60epoch')
    for iter, data in enumerate(train_loader):
        ...
        if iter % 20 == 0:
            ...
            cur_iter = epoch * len(train_loader)+ iter
            writer.add_scalar('loss', losses.avg, cur_iter)
            print(output_log)
            sys.stdout.flush()
    writer.close()
...
def main(args):
    ...
    if hasattr(cfg.train_cfg, 'pretrain'):
        ...
        # print(checkpoint['epoch'])#16
        # print(checkpoint['iter'])#0
    ...
    if args.resume:
        ...
        model.load_state_dict(checkpoint['state_dict'])
        optimizer.load_state_dict(checkpoint['optimizer'].keys())
...
if __name__ == '__main__':
    ...
    parser.add_argument('--config', help='config file path', type=str, default='config\pan\pan_r18_mlt.py')
